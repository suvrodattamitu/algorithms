...
public:
    ...
    void update(int idx, int low, int high, vector<int>& arr, int i, int val, int orr) {
        if(low == high) {
            seg[idx] = val;
            return;
        }
        ...
        int mid = (high + low) / 2;
        if(i <= mid) {
            update(2*idx+1, low, mid, arr, i, val, !orr);
        } else {
            update(2*idx+2, mid+1, high, arr, i, val, !orr);
        }
...
void solve() {
   ...
   int orr = 0;
   if(n%2) orr = 1;
   sg.build(0, 0, nn-1, arr, orr);
   
   int idx, val;
   while(m--) {
       get2(idx, val); --idx;
       sg.update(0, 0, nn-1, arr, idx, val, orr);
       put(sg.seg[0]);
   }
}
