...
void solve() {
    ...
    // total number of 1's in the number
    //if the number has no greater or equal 3 1's we will reduce it -1 to see if the previous number has 3 or more 1's in it
    ...
    ll ans = 0;
	int cnt = 0;
    // which bits has 1 for this number n
	for (int k = 61; k >= 0; k--) {
	    if (n & (1LL << k)) {
	        put(k);
    		ans |= 1LL << k;
