...
ll dp[300005][2];
ll rec(ll idx, ll health, vector<pair<ll, ll>>& arr) {
    if(idx == len(arr)) {
        return 0;
    }
    ...
    //not take it
    ll res = rec(idx+1, health, arr);

    //take it
    if(arr[idx].first == 0) {
        res = max(res, arr[idx].second + rec(idx+1, 0, arr));
    }
    
    else {
        if(!health) res = max(res, arr[idx].second + rec(idx+1, 1, arr));
    }
    
    return dp[idx][health] = res;
}
