...
string str1, str2;
ll lenFirst, lenSecon, dp[5005][5005];
ll rec(int ii, int jj) {
    if(ii == lenFirst) return (lenSecon - jj);
    if(jj == lenSecon) return (lenFirst - ii);
    ...
    if(str1[ii] == str2[jj]) {
        ...
        //delete or insert one character to the string.
        b = 1 + rec(ii+1, jj);
        c = 1 + rec(ii, jj+1);
        
        //Update/Replace one character in the string.
        d = 1 + rec(ii+1, jj+1);
    }
    
    return dp[ii][jj] = min(min(a, b), min(c, d));
}
