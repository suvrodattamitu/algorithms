...
ll dp[1000006];
ll rec(ll totalSum, vi& arr) {
    if(totalSum == 0) return 0;
    
    if(dp[totalSum] != -1) return dp[totalSum];
    
    ll ans = oo;
    for(ll coin : arr) {
        if(totalSum >= coin) {
            ans = min(ans, 1 + rec(totalSum - coin, arr));
        }
    }
    
    return dp[totalSum] = ans;
}

ll iterative(ll totalSum, vi& arr) {
    for (int curSum = 0; curSum <= totalSum; curSum++) { // state
        if (curSum == 0) {
            dp[curSum] = 0;
        } else {
            dp[curSum] = oo;
            for (int coin : arr) { // transition
                if (curSum >= coin) {
...
void solve() {
    ...
    // memo(dp);
    // ll ans = rec(totalSum, arr);

    ll ans = iterative(totalSum, arr);
    if(ans == oo) ans = -1;
    put(ans);
}
