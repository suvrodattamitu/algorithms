...
ll dp[50][10][3][3];
ll rec(string& number, int idx, int prev, bool leading_zeros, bool tight) {
    if(idx == len(number)) return 1;
    
    if(dp[idx][prev+1][leading_zeros][tight] != -1) return dp[idx][prev+1][leading_zeros][tight];

    ll ans = 0, up = tight ? (number[idx] - '0') : 9;
    for(int dig = 0; dig <= up; dig++) {
        if(dig == prev && leading_zeros == 0) continue;
        ans += rec(number, idx+1, dig, (leading_zeros & dig == 0), (tight & (dig == up)));
    }
    
    return dp[idx][prev+1][leading_zeros][tight] = ans;
}
...
void solve() {
    ...
    memo(dp);
    ull rightSide = rec(B, 0, -1, 1, 1);
    
    memo(dp);
    ull leftSide = rec(A, 0, -1, 1, 1);
    put(rightSide - leftSide);
}
