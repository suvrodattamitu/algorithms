...
ll n, x;
// dp[1005][100005]; // that gives runtime error so use 2d map instead
map<ll, map<ll, ll>> dp;

vi price, page;

ll rec(int idx, int cost) {
    if(idx == n) {
        if(cost <= x) return 0;
        return INT_MIN;
    }
    
    if(cost > x) return INT_MIN;
    
    if(dp[idx][cost] != 0) return dp[idx][cost];

    //do not take cur book
    ll not_take = rec(idx+1, cost);
    
    //take cur book
    ll take = page[idx] + rec(idx+1, cost + price[idx]);
    
    return dp[idx][cost] = max(not_take, take);
}
