...
ll dp[20][11][3][3];
ll rec(string& number, int idx, int prev, int leading_zeros, bool tight) {
    if(idx == 0) return 1;
    
    if(dp[idx][prev][leading_zeros][tight] != -1) return dp[idx][prev][leading_zeros][tight];

    ll ans = 0, up = tight ? (number[len(number) - idx] - '0') : 9;
...
void solve() {
    ...
    memo(dp);
    ll rightSide = rec(B, len(B), -1, 1, 1);
    
    memo(dp);
    ll leftSide = rec(A, len(A), -1, 1, 1);
