...
ll rec(ll i, int j, vi& arr, int player) {
    if(i >= j) return 0;

    ll fpSum1 = 0;
    if(player == 1) {
        if(i+1 < j) {
            fpSum1 = max(fpSum1, arr[i] + rec(i+1, j, arr, 2)); 
        }
        
        if(i < j-1) {
            fpSum1 = max(fpSum1, arr[j] + rec(i, j-1, arr, 2));
        }
    }
    ...
    if(player == 2) {
        if(i + 1 < j) {
            fpSum2 = max(fpSum2, arr[i] + rec(i+1, j, arr, 1));
        }
        
        if(i < j-1) {
            fpSum2 = max(fpSum2, arr[j] + rec(i, j-1, arr, 1));
        }
    }
    
    return max(fpSum1, fpSum2);  
}
